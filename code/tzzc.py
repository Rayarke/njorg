...
host = 'http://njna.nanjing.gov.cn/tzxq/tzzc/'
...
def getHref():
    url = ''
    res = urllib.request.urlopen('http://njna.nanjing.gov.cn/tzxq/tzzc/').read().decode('utf-8')
        ...
        if url[-5:] == '.html':
            ...
            if len(words) == 1:
                ...
                for tyxl_ncon_img in tyxl_ncon_imgs:
                    ...
                    # print(img_src)
                    folder = '/Users/schrodinger/njorg/tzzc_imgs'
                    dirt = Path(folder)
                    if dirt.exists():
                        wget.download(img_src, '/Users/schrodinger/njorg/tzzc_imgs')
                    else:
                        os.makedirs(folder)
                        wget.download(img_src, '/Users/schrodinger/njorg/tzzc_imgs')
            ...
            dirt = Path(folder)
            if dirt.exists():
                wget.download(url, '/Users/schrodinger/njorg/tzzc_imgs')
                article_content = url
            else:
                os.makedirs(folder)
                article_content = url
